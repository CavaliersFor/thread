 ...
 * 系统管理状态
 */
public enum EnumBizSystemStatus implements DataDictionaryEnum {
	/** 正常 */
	NORMAL(1, "正常", "green"),
	/** 冻结 */
...
//	LOCK(3,"关闭","red"),
	...
	private static Map<Integer, EnumBizSystemStatus> pool = new HashMap<Integer, EnumBizSystemStatus>();

	static {
		for (EnumBizSystemStatus each : EnumBizSystemStatus.values()) {
			EnumBizSystemStatus defined = pool.get(each.getValue());
			if (null != defined) {
	...
	EnumBizSystemStatus(int value, String text, String color) {
		this.value = value;
		this.text = text;
		this.color = color;
	...
	public static EnumBizSystemStatus valueOf(int value) {
