	...
	============================*/
(function(jQuery){
	$.fn.my_select=function(s_data,fn_){ 
	//重写s_data的参数，把所有的值初始化
	var s_data={
         select_class     :  s_data.select_class   ||  '',    //calss
         select_two       :  s_data.select_two     || '',     //二级开关
         select_three     :  s_data.select_three   || '',     //三级开关
         city_kiss        :  s_data.city_kiss      || ''
        
      }
      var s_obj = s_data.select_class;//select公用class
      var $_select_two = s_data.select_two;//select二级开关
      var $_select_three = s_data.select_three;//select三级开关
//全国数据，若要更换请传值覆盖，注意格式
var data_content_all=[
{name:'北京', cityList:[		   
...
//初始值
         var $_kiss_num = s_data.city_kiss;
		      if($_kiss_num !=''){
		      }else{
		      var $_kiss_num =['11','0','0'];//默认值设置
		      }
	       var A_  = data_content_all[$_kiss_num[0]];
          var A_0 = data_content_all[$_kiss_num[0]].name;
          var A_1 = data_content_all[$_kiss_num[0]].cityList[$_kiss_num[1]].name;
          var A_2 = data_content_all[$_kiss_num[0]].cityList[$_kiss_num[1]].areaList[$_kiss_num[2]];
          var B_1 = data_content_all[$_kiss_num[0]].cityList.length;
          var B_2 = data_content_all[$_kiss_num[0]].cityList[$_kiss_num[1]].areaList.length;
          $(s_obj).eq(0).append('<dt value="">'+A_0+'</dt>\n<dd style="display: none;">\n<ul></ul></dd>');
          $(s_obj).eq(1).append('<dt value=""title="'+A_1+'">'+A_1+'</dt>\n<dd style="display: none;">\n<ul></ul></dd>');
          $(s_obj).eq(2).append('<dt value="">'+A_2+'</dt>\n<dd style="display: none;">\n<ul></ul></dd>');
		 //转换dom文档
		 for(var n=0;n<$_kiss_num.length;n++){			
		 $(s_obj).eq(n).find('ul').empty();//清空 
		}
		for( var i=0; i<data_content_all.length; i++){
         var select_one =  $(s_obj).eq(0).find('ul'); 
         var value_a0 = 's_'+i;
		 select_one.append('<li><a href="#nogo" value="'+value_a0+'" class="my_select_01_all">'+data_content_all[i].name+'</a></li>');
	  	 }//初始导入值 
	  	 function ul_data_true(){
			 for(var k=0;k<B_1;k++){
			  var select_two =  $(s_obj).eq(1).find('ul');//循环查找 
			  var value_B1 = 'l_'+k; 
	          select_two.append('<li><a href="#nogo" value="'+value_B1+'" class="my_select_02_all" title="'+data_content_all[$_kiss_num[0]].cityList[k].name+'">'+data_content_all[$_kiss_num[0]].cityList[k].name+'</a></li>');
			 }
	         for(var g=0;g<B_2;g++){
	           var select_three =  $(s_obj).eq(2).find('ul'); 
	           var value_B2 = 't_'+g; 
	           select_three.append('<li><a href="#nogo" value="'+value_B2+'" class="my_select_03_all">'+data_content_all[$_kiss_num[0]].cityList[$_kiss_num[1]].areaList[g]+'</a></li>')
	         }
         }
         if($_select_two == true){ 
			     ul_data_true();//二三级初始数据
			 }
	  	 function change(){ 
		  	  var data_value02= $(s_obj).eq(1).children('dt');
		     var data_value03= $(s_obj).eq(2).children('dt');
	        var data_01_a_02 = $(s_obj).eq(1).children('dd');
	        var data_value_ul02 = data_01_a_02.find('ul');
	        var data_01_a_03 = $(s_obj).eq(2).children('dd');
	        var data_value_ul03 = data_01_a_03.find('ul');
	        var select_01_a_index =  $(s_obj).eq(0).children('dd').find('a');
	         select_01_a_index.each(function(index){
	         select_01_a_index.eq(index).click(function(){
            //alert(data_content_all[index].cityList.length)//二级长度
            data_value_ul02.empty();   //清空
            data_value_ul03.empty()    //清空
            var the_one_html =data_content_all[index].cityList[0].name;//初始值导入
            var the_two_html =data_content_all[index].cityList[0].areaList[0];//初始值导入
            data_value02.html(the_one_html).attr('value',''); //文案切换value清空
            data_value03.html(the_two_html).attr('value',''); //文案切换value清空
            for(var s=0; s<data_content_all[index].cityList.length; s++){
		           	 var value_a1 = 'l_'+s; 
		           data_value_ul02.append(
	            '<li><a href="#nogo" value="'+value_a1+'" class="my_select_02_all" title="'+data_content_all[index].cityList[s].name+'">'+data_content_all[index].cityList[s].name+'</a></li>'
		           );//导入值     
		           }
	        for(var d=0; d<data_content_all[index].cityList[0].areaList.length; d++){
	              var value_a2 = 't_'+d; 
	             data_value_ul03.append(
	    '<li><a href="#nogo" value="'+value_a2+'" class="my_select_03_all">'+data_content_all[index].cityList[0].areaList[d]+'</a></li>'
	       );//初始值导入值
	             }
		            function change_02(){
		             $('.my_select_02_all').each(function(index_02){
		              $('.my_select_02_all').eq(index_02).click(function(){
		               //alert(data_content_all[index].cityList[index_02].areaList.length)//三级长度
		                   data_value_ul03.empty()//清空
		                   var the_two_html =data_content_all[index].cityList[index_02].areaList[0];//初始值导入
                           data_value03.html(the_two_html).attr('value',''); //文案切换value清空
		              for(var d=0; d<data_content_all[index].cityList[index_02].areaList.length; d++){
	                      var value_a2 = 't_'+d; 
                         data_value_ul03.append(
	            '<li><a href="#nogo" value="'+value_a2+'" class="my_select_03_all">'+data_content_all[index].cityList[index_02].areaList[d]+'</a></li>'
		           );//导入值
	                     }
	                   })
	                  })
		            ...
		            //三级开关
			         if($_select_three == true && $_select_two == true){ 
			          change_02();//三级切换
			         }
             });
      ...
      //下拉功能模块  
		  $(s_obj).each(function(obj_index){
		      ...
		      dda.click(function(){
		        ...
		        //if($(s_obj).length==1&&(obj_index==0)){
		         fn_?fn_():null;
		        //}
		      })
            $(document).on('click','.my_select_02_all',function(){//二级菜单绑定事件
		        var $_dt_02 =  $(this).parents('dl').find('dt');
		        var $_dd_02 =  $(this).parents('dd');
		        $_dt_02.html($(this).html());
              $_dt_02.attr('title',$(this).html());
		        $_dt_02.attr('value',$(this).attr('value'));//value传值
		        $_dd_02.slideUp(200);
		        $_dt_02.removeClass('cur');
		        if(obj_index==1){
		        _ulheight_all();
		        }
		        //if($(s_obj).length==2&&(obj_index==1)){
		         fn_?fn_():null;
		        //}
		      })
            $(document).on('click','.my_select_03_all',function(){//三级菜单绑定事件
		        var $_dt_03 =  $(this).parents('dl').find('dt');
		        var $_dd_03 =  $(this).parents('dd');
		        $_dt_03.html($(this).html());
		        $_dt_03.attr('value',$(this).attr('value'));//value传值
		        $_dd_03.slideUp(200);
		        $_dt_03.removeClass('cur');
		         //if($(s_obj).length==3&&(obj_index==2)){
		           fn_?fn_():null;
		         //}
		      })
