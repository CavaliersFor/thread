 ...
 * 
 * <p>
 * create at 2016年6月16日 下午7:58:49
 * 
...
public class HtmlCompressor {
	...
	private static String fix(String html) {
		...
		if (metaIndex > 0 && !html.contains("</meta>")) {
			String pre = html.substring(0, metaIndex);
			String arf = html.substring(metaIndex, htmlLength);
			arf = arf.substring(arf.indexOf(">") + 1, arf.length());
			html = pre + arf;
		...
		if (!html.contains("<html xmlns=\"http://www.w3.org/1999/xhtml\">")) {
			...
			if (styleIndex > 0) {
				sb.append(html.substring(0, styleIndex + 8));
			}
			sb.append("</head>");
			sb.append("<body>");
			if (styleIndex > 0) {
				sb.append(html.substring(styleIndex + 8, htmlLength));
			} else {
				sb.append(html);
			}
